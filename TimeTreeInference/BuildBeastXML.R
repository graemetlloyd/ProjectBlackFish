PrepareBeastMetatree<- function(constraintTree, StratRanges, alignment, myfileName, makeStartTree = TRUE, start.tree.method="mbl", vartime=0.1) {
  ...
  ## remove strat data not in tree ##
  #StratRanges <- StratRanges[-seq(1,nrow(StratRanges))[-sort(match(constraintTree$tip.label,rownames(StratRanges)))], ]
  
  ## remove things with missing data ##
  # if(!length(is.na(StratRanges[,1]))==0) {
